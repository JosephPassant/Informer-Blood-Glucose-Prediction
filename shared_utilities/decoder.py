...
class Decoder(nn.Module):
    ...
    def forward(self, x, cross, x_mask=None, cross_mask=None):
        ...
        if x.shape[1] > cross.shape[1]:  # If decoder input is longer than encoder output
            cross = self.conv_downsample(cross.permute(0, 2, 1)).permute(
                0, 2, 1)  # Apply Conv1D & reshape
        ...
        if cross.shape[1] > x.shape[1]:
            ...
            pad = torch.zeros(
                (cross.shape[0], x.shape[1] - cross.shape[1], cross.shape[2]), device=cross.device)
