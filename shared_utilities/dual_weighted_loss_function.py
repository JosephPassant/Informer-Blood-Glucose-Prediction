...
class P_EGA_Loss:
    ...
    def __init__(self, y_true, dy_true, y_pred):
        ...
        assert len(self.y_true) == len(self.y_pred) == len(
            self.dy_true), "Mismatch in input array lengths."
    ...
    def full(self):
        ...
        # Expand upper for falling rates
        mod[(self.dy_true >= -2) & (self.dy_true <= -1)] = 10
        # Expand lower for rising rates
        ...
        classifications = np.zeros(
            (num_samples, 5), dtype=int)  # A, B, C, D, E
...
class R_EGA_Loss:
    ...
    def full(self):
        ...
        # Upper E: True falling, predicted rising
        uE_mask = (self.dy_pred > 1) & (self.dy_true < -1)
        # Lower E: True rising, predicted falling
        ...
        classifications[A_mask & ~uE_mask & ~lE_mask & ~
                        uD_mask & ~lD_mask & ~uC_mask & ~lC_mask, 0] = 1  # A

        # B zone (Benign Rate Error) - everything else
        B_mask = ~(uE_mask | lE_mask | uD_mask |
                   lD_mask | uC_mask | lC_mask | A_mask)
        classifications[B_mask, 1] = 1  # B
...
class CG_EGA_Loss:
    ...
    def __init__(self, y_true, dy_true, y_pred, dy_pred, freq=5):
        ...
        assert len(self.y_true) == len(
            self.y_pred), f"Length mismatch: y_true ({len(self.y_true)}) != y_pred ({len(self.y_pred)})"
        assert len(self.dy_true) == len(
            self.dy_pred), f"Length mismatch: dy_true ({len(self.dy_true)}) != dy_pred ({len(self.dy_pred)})"
    ...
    def simplified(self):
        ...
        classifications = np.array([
            self.map_cg_ega(
                p, r, "hypo" if hypo_mask[i] else "eu" if eu_mask[i] else "hyper")
    ...
    def reduced(self):
        ...
        sum_counts = counts["count_hypo"] + \
            counts["count_eu"] + counts["count_hyper"]
...
class CGEGALoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        for i in range(batch_size):
            ...
            y_true = target_glucose[i].detach(
            ...
            try:
                ...
                weights[i] = torch.tensor(
                    sample_weights, device=weights.device, dtype=weights.dtype)
                ...
                try:
                    ...
                    moderate_errors = (~severe_errors) & (
                        (rel_error > 0.15) | (abs_error > 15))
                    ...
                    print(
                        f"Fallback  failed: {str(e)}. Using basic heuristic.")
                    hypo_mask = (target_glucose[i] <= 70).cpu().numpy()
                    sample_weights = np.ones_like(y_true, dtype=np.float32)
                    sample_weights[hypo_mask] = self.EP_weight * \
                        self.hypo_multiplier
                    weights[i] = torch.tensor(
                        sample_weights, device=weights.device, dtype=weights.dtype)
