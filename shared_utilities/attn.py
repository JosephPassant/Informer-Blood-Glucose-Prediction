...
class MicroscaleAttention(nn.Module):
    def __init__(self, d_model, n_heads=8, window_size=12, overlap=6, dropout=0.1, mask_flag=False):
        ...
        self.proj_q = nn.Linear(
            self.head_dim, self.head_dim, bias=False)  # ✅ Correct
    ...
    def forward(self, queries, keys, values, attn_mask=None):
        ...
        # Unfold along sequence length (L)
        queries_unfold = queries.unfold(2, self.window_size, step)

        # (B, H, num_windows, window_size, D)
        queries_unfold = queries_unfold.permute(0, 1, 2, 4, 3)


        keys_unfold = keys.permute(0, 2, 1, 3).unfold(
            2, self.window_size, step)
        # (B, H, num_windows, window_size, D)
        keys_unfold = keys_unfold.permute(0, 1, 2, 4, 3)

        values_unfold = values.permute(
            0, 2, 1, 3).unfold(2, self.window_size, step)
        
        # (B, H, num_windows, window_size, D)
        values_unfold = values_unfold.permute(0, 1, 2, 4, 3)
        ...
        attn_output = attn_output.permute(0, 2, 3, 1, 4).reshape(
            B, num_windows, window_size, H * head_dim)


        # Aggregate Overlapping Windows 
        # Ensure correct length for `output` to match attention mechanism's sequence length
        output = torch.zeros(B, self.window_size + step *
                             (num_windows - 1), H * head_dim, device=queries.device)
        count = torch.zeros(B, self.window_size + step *
                            (num_windows - 1), device=queries.device)
        ...
        correct_L = self.window_size + step * \
            (num_windows - 1)  # Compute correct sequence length
        ...
        # Ensure queries are (B, L, H * D)
        queries = queries.reshape(B, L, H * D)
...
class DynamicMicroscaleAttention(nn.Module):
    def __init__(self, d_model, n_heads=8, window_size=6, overlap=3, dropout=0.1, mask_flag=False):
        ...
        self.localtemperature = nn.Parameter(
            torch.sigmoid(torch.ones(1)) + 0.1)
        self.globaltemperature = nn.Parameter(
            torch.sigmoid(torch.ones(1)) + 0.1)
        # Learnable weight for fusing local and global attention outputs
        self.alpha = nn.Parameter(torch.sigmoid(
            torch.ones(1)))  # Constrained to [0,1]
    ...
    def forward(self, queries, keys, values, attn_mask=None):
        ...
        # Unfold along sequence length (Microscale Attention)
        queries_unfold = queries.unfold(
            1, self.window_size, step).permute(0, 1, 4, 2, 3)
        keys_unfold = keys.unfold(
            1, self.window_size, step).permute(0, 1, 4, 2, 3)
        values_unfold = values.unfold(
            1, self.window_size, step).permute(0, 1, 4, 2, 3)
        ...
        #  Compute Microscale Attention Scores (WITH Temperature Adjustment)
        attn_scores_micro = torch.matmul(
            Q_micro, K_micro.transpose(-2, -1)) / self.localtemperature
        ...
        #  Weighted Local Aggregation (From Microscale Attention) 
        ...
        for i in range(num_windows):
            ...
            local_weights = attn_weights_micro[:, :, i, :, :].mean(
                dim=1)  # (B, window_size, window_size)
            ...
            attn_output_window = attn_output_micro[:, :, i, :, :].reshape(
                B, H, window_size, head_dim)
            attn_output_window = attn_output_window.permute(
                0, 2, 1, 3).reshape(B, window_size, H * head_dim)
        ...
        #  Compute Global Attention (DAGA) 
        Q_global = self.global_proj_q(queries.reshape(
            B, L, H * head_dim))  # Flatten heads
        K_global = self.global_proj_k(keys.reshape(B, L, H * head_dim))
        V_global = self.global_proj_v(values.reshape(B, L, H * head_dim))

        attn_scores_global = torch.matmul(
            Q_global, K_global.transpose(-2, -1)) / self.globaltemperature
        ...
        #  Fuse Local and Global Attention Outputs Using Alpha 
        fused_output = self.alpha * output + \
            (1 - self.alpha) * attn_output_global
