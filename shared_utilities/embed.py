...
class PositionalEmbedding(nn.Module):
    def __init__(self, d_model, max_len=72):
...
class TemporalEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        minute_x = self.minute_embed(x[:, :, 1]) if hasattr(
...
class TimeFeatureEmbedding(nn.Module):
    """UPDATED FOR GLUCOSE FORECSATING"""

    def __init__(self, d_model, embed_type='timeF', freq='h'):
        super(TimeFeatureEmbedding, self).__init__()

        freq_map = {'h': 2, 't': 2}  # Only 2 inputs: hour and minute
        d_inp = freq_map[freq]
...
class DataEmbedding(nn.Module):
    def __init__(self, c_in, d_model, embed_type='timef', freq='h', dropout=0.1):
        ...
        self.hour_min_index = c_in - 2

    def forward(self, x):

        x_mark = x[:, :, self.hour_min_index:]  # Hour & Minute
        x = x[:, :, :self.hour_min_index]  # Glucose Values features
