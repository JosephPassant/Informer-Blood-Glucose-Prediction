...
def validate(val_loader, model, include_cg_ega=True, mean=152.91051040286524, std=70.27050122812615):
        ...
        for batch_x, batch_dec, batch_y in val_loader:
            ...
            if include_cg_ega:
                ...
                for i in range(len(outputs)):
                    ...
                    y_pred = pred_glucose[i].numpy().astype(
                        np.float32).flatten()
                    y_true = target_glucose[i].numpy().astype(
                        np.float32).flatten()
                    ...
                    try:
                        cg_ega = CG_EGA_Loss(
                            y_true, dy_true, y_pred, dy_pred, freq=5)
                        ...
                        for region in ["hypo", "eu", "hyper"]:
                            if counts[f"count_{region}"] > 0:
                                for category in ["AP", "BE", "EP"]:
                                    locals()[
                                        f"{region}_stats"][category] += results[f"{category}_{region}"]
                                locals()[
                                    f"{region}_stats"]["count"] += counts[f"count_{region}"]
                    except Exception as e:
                        print(
                            f"CG-EGA calculation failed: {str(e)}. Skipping this sample.")
    ...
    total_samples = hypo_stats["count"] + \
        eu_stats["count"] + hyper_stats["count"]
    ...
    if include_cg_ega:
        ...
        for region, stats in [("Hypo", hypo_stats), ("Eu", eu_stats), ("Hyper", hyper_stats)]:
            if stats["count"] > 0:
                ...
                print(
                    f"  {region}: AP={ap_pct:.1f}%, BE={be_pct:.1f}%, EP={ep_pct:.1f}% (n={stats['count']})")

        print(
            f"Overall: AP={ap_percent:.1f}%, BE={be_percent:.1f}%, EP={ep_percent:.1f}% (n={total_samples})")
...
def train(train_iter, model, optimizer, lr_scheduler, config, ap_weight, be_weight, ep_weight, hypo_multiplier, train_loader, val_loader, save_dir):
    ...
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device(
        'mps') if torch.backends.mps.is_available() else torch.device('cpu')
    # Lower is better for erroneous prediction percentage
    best_ep_percent = float("inf")
    best_rmse = float("inf")
    best_model_state_dict = None
    validation_metrics_table = []

    # Create loss function once
    loss_fn = CGEGALoss(AP_weight=ap_weight, BE_weight=be_weight,
                        EP_weight=ep_weight, hypo_multiplier=hypo_multiplier)
    print(
        f"Using CG-EGA Loss with AP={ap_weight}, BE={be_weight}, EP={ep_weight}, Hypo={hypo_multiplier}")
    for epoch in range(config.train_epochs):
        ...
        print(
            f"Epoch {epoch+1} | Train Loss: {avg_epoch_loss:.6f} | EP%: {ep_percent:.2f}% | RMSE: {val_rmse:.6f}")
        ...
        if ep_percent < best_ep_percent:
            ...
            print(
                f"New best model found at epoch {epoch+1} with EP%: {ep_percent:.2f}% (RMSE: {val_rmse:.6f})")

    if best_model_state_dict:
        best_model_path = os.path.join(
            save_dir, f"best_validation_model_rmse_{best_ep_percent:.2f}.pth")
        torch.save(best_model_state_dict, best_model_path)
        print(
            f"Best model saved at {best_model_path} with RMSE: {best_ep_percent:.2f}")
    else:
